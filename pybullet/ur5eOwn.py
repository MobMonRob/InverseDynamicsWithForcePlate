...
class UR5Sim():
    ...
    def load_robot(self):
        ...
        robot = pybullet.loadURDF(ROBOT_URDF_PATH, [0, 0, 0], [0, 0, 0, 1], flags=flags, useFixedBase = 1 )
        return robot
...
def simulate_movements():
    ...
    next_joint_angle = math.radians(45)
    while True:
        x, y, z, Rx, Ry, Rz = sim.read_gui_sliders()
        
        # Update joint_angles for the current cycle
        joint_angles = [base_angle, next_joint_angle, 0, 0, 0, 0]
        
        sim.set_joint_angles(joint_angles)
        sim.check_collisions()
        
        # Update joint_angles for the next cycle
        joint_angles = [math.radians(-180), math.radians(-45), 0, 0, 0, 0]
        
        # Swap base_angle and next_joint_angle for the next cycle
        base_angle, next_joint_angle = joint_angles[0], joint_angles[1]
